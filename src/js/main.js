...
$(document).ready(function () {
	...
	const isContentEmpty = $content.length === 0 || $.trim($content.text()) === "";
...
function animateCursor() {
	...
	mouseInner.style.transform = `translate(${innerX}px, ${innerY}px) translate(-50%, -50%)`;
...
// Optional: click hiệu ứng
window.addEventListener("mousedown", () => {
	mouseOuter.style.transform += " scale(0.7)";
});
window.addEventListener("mouseup", () => {
	mouseOuter.style.transform = mouseOuter.style.transform.replace(/scale\(0\.7\)/, "");
});
